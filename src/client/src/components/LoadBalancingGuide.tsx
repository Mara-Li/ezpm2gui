  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-[11px] uppercase tracking-[0.2em] text-[#555] mb-1">pm2 / docs</p>
          <h1 className="text-sm font-bold text-[#e8e8e8]">▸ LOAD BALANCING GUIDE</h1>
          <p className="text-[12px] text-[#555] mt-0.5">{t('loadBalancing.subtitle')}</p>
        </div>
        <button
      ...
      <div className="bg-[#111] border border-[#1e1e1e] rounded-sm overflow-hidden">
        ...
        >
          <span className="text-[13px] font-bold text-[#e8e8e8] uppercase tracking-[0.05em]">
            {t('loadBalancing.whatIsTitle')}
          </span>
          <span className="text-[#555] text-xs">{expanded === 'panel1' ? '▾' : '▸'}</span>
        </button>
        {expanded === 'panel1' && (
          <div className="px-4 pb-4 border-t border-[#1e1e1e]">
            <p className="text-[12px] text-[#888] leading-relaxed mt-3 mb-3">
            ...
            <ul className="space-y-1.5">
              {[
                ...
                <li key={text} className="flex items-start gap-2">
                  <span className={`text-[11px] mt-0.5 ${color}`}>{symbol}</span>
                  <span className="text-[12px] text-[#888] leading-relaxed">{text}</span>
                </li>
              ))}
            </ul>
          </div>
      ...
      <div className="bg-[#111] border border-[#1e1e1e] rounded-sm overflow-hidden">
        ...
        >
          ...
          <span className="text-[#555] text-xs">{expanded === 'panel2' ? '▾' : '▸'}</span>
        </button>
        {expanded === 'panel2' && (
          <div className="px-4 pb-4 border-t border-[#1e1e1e]">
            <p className="text-[12px] text-[#888] leading-relaxed mt-3 mb-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {[
                ...
                <div key={title} className="bg-[#141414] border border-[#1e1e1e] rounded-sm p-3">
                  <p className="text-[13px] font-bold text-[#e8e8e8] uppercase tracking-[0.05em] mb-1">{title}</p>
                  <p className="text-[12px] text-[#888] leading-relaxed mb-2">{desc}</p>
                  <pre className="bg-[#0a0a0a] border border-[#1e1e1e] font-mono text-[12px] text-[#22d3ee] p-3 rounded-sm whitespace-pre-wrap">
                    {code}
                  </pre>
                </div>
              ))}
      ...
      <div className="bg-[#111] border border-[#1e1e1e] rounded-sm overflow-hidden">
        ...
        >
          ...
          <span className="text-[#555] text-xs">{expanded === 'panel3' ? '▾' : '▸'}</span>
        </button>
        {expanded === 'panel3' && (
          <div className="px-4 pb-4 border-t border-[#1e1e1e]">
            <ul className="space-y-3 mt-3">
              {[
                ...
                <li key={primary} className="flex items-start gap-3">
                  <span className="text-[11px] text-[#f59e0b] mt-0.5">▸</span>
                  <div>
                    <p className="text-[12px] font-bold text-[#e8e8e8]">{primary}</p>
                    <p className="text-[12px] text-[#888] leading-relaxed">{secondary}</p>
                  </div>
                </li>
              ))}
            </ul>
