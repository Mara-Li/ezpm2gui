  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-[11px] uppercase tracking-[0.2em] text-[#555] mb-1">pm2 / tools</p>
          <h1 className="text-sm font-bold text-[#e8e8e8]">▸ ECOSYSTEM GENERATOR</h1>
          <p className="text-[12px] text-[#555] mt-0.5">{t('ecosystem.subtitle')}</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => generateEcosystem(true)}
            disabled={loading}
            className="border border-[#333] text-[#888] text-xs px-3 py-1 rounded-sm hover:border-[#555] hover:text-[#aaa] disabled:opacity-40 disabled:cursor-not-allowed"
          >
            {t('common.preview')}
          </button>
          <button
            ...
            disabled={loading}
            className="bg-[#e8e8e8] text-[#0a0a0a] text-xs font-semibold px-4 py-1.5 rounded-sm hover:bg-[#ccc] disabled:opacity-40 disabled:cursor-not-allowed"
          >
      ...
      <div className="bg-[#111] border border-[#1e1e1e] rounded-sm p-4">
        <p className="text-[11px] text-[#555] uppercase tracking-[0.2em] mb-3 block">
          {t('ecosystem.outputOptions')}
        </p>

        <form onSubmit={e => { e.preventDefault(); generateEcosystem(false); }} className="space-y-3">
          <div>
            <label className="text-[11px] text-[#555] uppercase tracking-[0.2em] mb-1.5 block">
            ...
            />
            <p className="text-[11px] text-[#555] mt-1">{t('ecosystem.savePathHelper')}</p>
          </div>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-[12px] text-[#888]">{t('ecosystem.includeStopped')}</span>
          </label>
        </form>

        <p className="text-[11px] text-[#555] mt-4 leading-relaxed border-t border-[#1e1e1e] pt-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
          <div className="bg-[#111] border border-[#1e1e1e] rounded-sm w-full max-w-2xl mx-4 flex flex-col max-h-[80vh]">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-[#1e1e1e]">
              <span className="text-[12px] font-bold text-[#e8e8e8]">ecosystem.config.js — Preview</span>
              <button
            ...
            <div className="flex items-center justify-end gap-2 px-4 py-3 border-t border-[#1e1e1e]">
              <button
                onClick={handleCopy}
                className="border border-[#333] text-[#888] text-xs px-3 py-1 rounded-sm hover:border-[#555]"
              >
                {copied ? 'copied' : 'copy'}
              </button>
              <button
                onClick={() => setPreviewOpen(false)}
                className="border border-[#1e1e1e] text-[#555] text-xs px-3 py-1 rounded-sm hover:border-[#333] hover:text-[#888]"
              >
                {t('common.close')}
              </button>
              <button
                onClick={() => { setPreviewOpen(false); generateEcosystem(false); }}
                className="bg-[#e8e8e8] text-[#0a0a0a] text-xs font-semibold px-4 py-1.5 rounded-sm hover:bg-[#ccc]"
              >
