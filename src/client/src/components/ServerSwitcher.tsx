        ...
        <div className="absolute top-full mt-1.5 right-0 w-60 bg-[#111] border border-[#1e1e1e] rounded-sm shadow-2xl z-50 py-1">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[11px] text-[#555] mt-0.5">{t('serverSwitcher.localhost')}</div>
            </div>
            {activeServerId === 'local' && (
              <CheckIcon className="h-3.5 w-3.5 shrink-0 text-[#555]" />
            )}
          </button>
          ...
          {connections.length > 0 && (
            <>
              ...
              {connections.map(c => (
                ...
                >
                  {c.connected
                    ? <SignalIcon className="h-4 w-4 shrink-0 text-[#22c55e]" />
                    : <SignalSlashIcon className="h-4 w-4 shrink-0 text-[#555]" />
                  }
                  <div className="flex-1 min-w-0">
                    <div className="truncate">{c.name}</div>
                    <div className="text-[11px] text-[#555] truncate mt-0.5">{c.username}@{c.host}</div>
                  </div>
                  {activeServerId === c.id && (
                    <CheckIcon className="h-3.5 w-3.5 shrink-0 text-[#555]" />
                  )}
