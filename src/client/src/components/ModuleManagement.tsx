...
const statusBadge = (status: string): string => {
  ...
  if (s.includes('online') || s.includes('enabled'))
    return 'text-[11px] text-[#22c55e] border border-[#22c55e]/30 bg-[#022c00] px-1.5 py-0.5 rounded-sm';
  if (s.includes('error') || s.includes('disabled'))
    return 'text-[11px] text-[#ef4444] border border-[#ef4444]/30 bg-[#1a0000] px-1.5 py-0.5 rounded-sm';
  return 'text-[11px] text-[#f59e0b] border border-[#f59e0b]/30 bg-[#1a0e00] px-1.5 py-0.5 rounded-sm';
};
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative z-10 flex flex-col w-full max-w-md h-full
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-[#1e1e1e] shrink-0">
          <div>
            <p className="text-[11px] uppercase tracking-[0.2em] text-[#555] mb-0.5">▸ CONFIGURE MODULE</p>
            <p className="text-[13px] font-bold text-[#e8e8e8]">{moduleName}</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          {loading ? (
            <div className="flex items-center justify-center h-24 gap-2 text-xs text-[#555]">
              <svg className="h-5 w-5 animate-spin text-[#888]" viewBox="0 0 24 24" fill="none">
            ...
            <>
              ...
              {schema.length > 0 && (
                <div className="space-y-3">
                  <p className="text-[11px] uppercase tracking-[0.2em] text-[#555]">▸ SETTINGS</p>
                  {schema.map(field => {
                      ...
                      <div key={field.key}>
                        ...
                        {field.type === 'boolean' ? (
                          <div className="flex items-center gap-2">
                            ...
                            >
                              <span className={`pointer-events-none inline-block h-5 w-5 rounded-sm bg-[#0a0a0a] shadow
                                               transform transition-transform duration-200
                                               ${val === 'true' ? 'translate-x-4' : 'translate-x-0'}`} />
                            </button>
                            <span className="text-[12px] text-[#555]">{val === 'true' ? 'enabled' : 'disabled'}</span>
                          </div>
                        ) : (
                          <input
                        ...
                        {field.description && (
                          <p className="mt-0.5 text-[12px] text-[#444]">{field.description}</p>
                        )}
                      </div>
                    );
                  })}
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-[12px] text-[#888] hover:text-[#e8e8e8] transition-colors"
                  >
                    + add key
                  </button>
                </div>
                ...
                {extras.length === 0 && schema.length > 0 ? (
                  <p className="text-[12px] text-[#444] italic">no additional keys</p>
                ) : (
                  extras.map((e, i) => (
              ...
              {schema.length === 0 && extras.length === 0 && (
                <p className="text-[12px] text-[#444] italic text-center py-4">
        ...
        <div className="flex justify-end gap-2 px-4 py-3 border-t border-[#1e1e1e] shrink-0">
          <button
            onClick={onClose}
            className="h-7 px-3 text-xs border border-[#1e1e1e] text-[#555] hover:text-[#888] hover:border-[#333] rounded-sm transition-colors"
          >
            cancel
          </button>
          <button
            ...
            className="h-7 px-3 bg-[#e8e8e8] text-[#0a0a0a] text-xs font-semibold rounded-sm
  ...
  return (
    <div>
        ...
        actions={
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={loading}
              className="h-7 px-3 text-xs border border-[#1e1e1e] text-[#555] hover:text-[#888] hover:border-[#333]
      ...
      <section className="mb-6">
        <p className="text-[11px] uppercase tracking-[0.2em] text-[#555] mb-2">▸ INSTALLED</p>
        ...
        <div className="border border-[#1e1e1e] rounded-sm overflow-hidden bg-[#111]">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-24 gap-1 text-[#555]">
              <PuzzlePieceIcon className="h-6 w-6 opacity-40" />
              <span className="text-[12px]">no PM2 modules installed</span>
            </div>
          ) : (
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-[#1e1e1e] bg-[#0d0d0d] text-[#555]">
                  <th className="px-3 py-2 text-left text-[11px] uppercase tracking-[0.15em]">Name</th>
                  <th className="px-3 py-2 text-left text-[11px] uppercase tracking-[0.15em]">Version</th>
                  <th className="px-3 py-2 text-left text-[11px] uppercase tracking-[0.15em]">Status</th>
                  <th className="px-3 py-2 text-right text-[11px] uppercase tracking-[0.15em]">Actions</th>
                </tr>
              </thead>
              <tbody>
                {modules.map((mod, i) => (
                  ...
                  >
                    <td className="px-3 py-2 font-bold text-[13px] text-[#e8e8e8]">{mod.name}</td>
                    <td className="px-3 py-2 text-[12px] text-[#555]">{mod.version}</td>
      ...
      <section>
        <p className="text-[11px] uppercase tracking-[0.2em] text-[#555] mb-2">▸ AVAILABLE MODULES</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {KNOWN_MODULES.map(mod => {
            ...
            return (
              ...
              >
                <div className="shrink-0 mt-0.5 h-8 w-8 rounded-sm flex items-center justify-center bg-[#1a1a1a]">
                  <PuzzlePieceIcon className="h-5 w-5 text-[#444]" />
                </div>

                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className="text-[13px] font-bold text-[#e8e8e8]">{mod.name}</span>
                    <span className={`text-[11px] px-1.5 py-0.5 rounded-sm ${categoryColor[mod.category] ?? categoryColor.Utilities}`}>
                      {mod.category}
                    </span>
                    {isInstalled && (
                      <span className="text-[11px] text-[#22c55e] border border-[#22c55e]/30 bg-[#022c00] px-1.5 py-0.5 rounded-sm">
                        installed
                      </span>
                    )}
                  </div>
                  <p className="text-[12px] text-[#555] mt-0.5 leading-relaxed">{mod.description}</p>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70">
          <div className="w-full max-w-sm bg-[#111] border border-[#1e1e1e] rounded-sm shadow-2xl">
            <div className="flex items-center justify-between px-4 py-3 border-b border-[#1e1e1e]">
              <p className="text-xs text-[#e8e8e8] font-semibold">▸ INSTALL PM2 MODULE</p>
            ...
            <div className="px-4 py-4">
              <label className="block text-[12px] text-[#555] mb-1">Module Name</label>
              <input
                autoFocus
                type="text"
                value={newModuleName}
                ...
                className="w-full h-8 px-3 bg-[#0d0d0d] border border-[#1e1e1e] text-[#e8e8e8] text-xs rounded-sm
                           placeholder-[#333] focus:outline-none focus:border-[#333]"
              />
              <p className="mt-1.5 text-[12px] text-[#444]">enter any npm-published PM2 module name</p>
            </div>
